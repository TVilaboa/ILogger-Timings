...
namespace ILoggerTimings
{
    ...
    public class Operation : IDisposable
    {
        ...
        const string OutcomeCompleted = "completed", OutcomeAbandoned = "abandoned", OutcomeFailed = "failed";

        readonly ILogger _target;
        readonly string _messageTemplate;
        readonly IDictionary<string, object> _args;
        ...
        internal Operation(ILogger target, string messageTemplate, IDictionary<string,object> args, CompletionBehaviour completionBehaviour, LogLevel completionLevel, LogLevel abandonmentLevel)
        {
            ...
            //_context = new Dictionary<string, object> {[nameof(Properties.OperationId)] = Guid.NewGuid()};
            //args[nameof(Properties.OperationId)] = Guid.NewGuid();
            _popContext = target.BeginScope(new Dictionary<string,object>(){{ nameof(Properties.OperationId), Guid.NewGuid() },{"Context",args} });
        ...
        public void Dispose()
        {
            ...
            switch (_completionBehaviour)
            {
                ...
                case CompletionBehaviour.Complete:
                    string outcome=OutcomeCompleted;
                    var level = _completionLevel;
        ...
        void Write(ILogger target, LogLevel level, string outcome)
        {
            ...
            //var message = ($"{_messageTemplate} {{{nameof(Properties.Outcome)}}} in {{{nameof(Properties.Elapsed)}:0.0}} ms", _args.Concat(
            //new object[] { outcome, elapsed }).ToArray());
            //var message = $"{_messageTemplate} {{{nameof(Properties.Outcome)}}} in {{{nameof(Properties.Elapsed)}:0.0}} ms";
            //target.Log(level, $"{_messageTemplate} {outcome} in {elapsed} ms", _context,_args);
            //target.Log(level, $"{_messageTemplate} {outcome} in {elapsed} ms", _args.Concat(
            //    new object[] { outcome, elapsed }).ToArray());
            target.Log(level, $"{_messageTemplate} {outcome} in {elapsed} ms");
